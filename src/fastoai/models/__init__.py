...
def generate_module(cls: type[BaseModel]):
    ...
    regular_imports.append((cls.__module__, (f"{name} as _{name}",)))
    ...
    object_match = re.search(object_pattern, base)
...
"""
    ...
    if len(relationships):
        text += "\n" + indent("\n".join(relationships), "    ") + "\n"
    text += f"""
    def to_openai_model(self) -> _{name}:
        value = self.model_dump()
        value["object"] = "{object_match.group(1)}"
        return _{name}.model_validate(value)
"""
